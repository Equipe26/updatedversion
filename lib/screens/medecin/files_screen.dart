...
class _FilesScreenState extends State<FilesScreen> {
  ...
  Future<void> _loadPatients() async {
    try {
      ...
      setState(() {
        _patients = ['Tous', ...patients.map((p) => p.name)];
        print('Patients chargés: $_patients');
      });
    } catch (e) {
  ...
  Future<void> _loadDocuments() async {
    try {
      ...
      if (user != null) {
        ...
        // Précharger les noms des patients
        final patientIds = documents
        ...
        for (final id in patientIds) {
          final name = await _patientService.getPatientName(id!);
          if (name != null) {
            _patientNameCache[id] = name;
          }
        }
  ...
  String _getPatientNameFromDocument(MedicalDocument doc) {
    if (doc.patientId != null && _patientNameCache.containsKey(doc.patientId)) {
      return _patientNameCache[doc.patientId]!;
    }
    return 'Patient ${doc.patientId?.substring(0, 6) ?? 'inconnu'}';
  }
  ...
  String get _activeFilterText {
    switch (_selectedFilterIndex) {
      ...
      case 1:
        return _selectedPatient ?? _filters[1];
      case 2:
                        ...
                        child: SingleChildScrollView(
                          child: Column(
                            ...
                            children: _patients.map((patient) {
                              return ListTile(
                                title: Text(patient),
                                trailing: _selectedPatient == patient
                                    ? Icon(Icons.check, color: primaryColor)
                                    : null,
                                onTap: () {
                                  if (!mounted) return;
                                  setState(() {
                                    _selectedPatient = patient == 'Tous' ? null : patient;
                                    _removeOverlay();
                                    _applyFilters();
                                  });
                                },
  ...
  void _applyFilters() {
    ...
    if (_selectedPatient != null && _selectedPatient != 'Tous') {
      filteredDocuments = filteredDocuments
          .where((doc) => _getPatientNameFromDocument(doc) == _selectedPatient)
          .toList();
    }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 4),
                      Text(
                        _getPatientNameFromDocument(document),
